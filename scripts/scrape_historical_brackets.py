...
def scrape_seed_matchups():
    ...
    for url in WIKI_URLS:
        ...
        for table in tables:
            ...
            if not any(
                kw in (caption_text + header_text).lower()
                for kw in ["seed", "round of 64", "first round", "matchup"]
            ):
                continue

            print(
                f"  Parsing table: {caption_text[:60] if caption_text else header_text[:60]}..."
            )
...
def main():
    ...
    for row in rows:
        if row["round"] == "R64":
            print(
                f"  {row['higher_seed']} vs {row['lower_seed']}: "
