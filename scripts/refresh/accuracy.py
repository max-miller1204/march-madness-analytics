...
class AccuracyTracker:
    ...
    def __init__(self, log_path=None):
        self.log_path = log_path or str(REPO_ROOT / 'scraped_data' / 'accuracy_log.json')
    ...
    def log_run(self, predictions, completed_games, round_name=None):
        ...
        if round_name is None:
            rounds_seen = [p.get('round', 'Unknown') for p in predictions if p['game_id'] in outcomes]
    ...
    def generate_dashboard(self, output_path=None):
        ...
        if not runs:
            print("[AccuracyTracker] No runs to plot. Generating placeholder dashboard.")
    ...
    def _generate_empty_dashboard(self, output_path):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle('Prediction Accuracy Dashboard (No Data Yet)', fontsize=16, fontweight='bold')
        for ax in axes.flat:
    ...
    def _plot_accuracy_by_round(self, ax, runs):
        ...
        ax.bar(x, incorrect_vals, width, bottom=correct_vals, label='Incorrect', color='#e74c3c')
        ax.set_xticks(x)
    ...
    def _plot_calibration_curve(self, ax, runs):
        ...
        for run in runs:
            for pred in run.get('predictions', []):
                all_preds.append((pred['predicted_prob'], 1.0 if pred['correct'] else 0.0))
        ...
        ax.plot(bin_centers, bin_actuals, 'o-', color='#3498db', markersize=6, label='Model')

        # Annotate bin counts
        for x_val, y_val, count in zip(bin_centers, bin_actuals, bin_counts):
            ax.annotate(f'n={count}', (x_val, y_val), textcoords='offset points',
                        xytext=(0, 8), fontsize=7, ha='center', color='gray')
    ...
    def _plot_brier_over_time(self, ax, runs):
        ...
        ax.axhline(y=0.25, color='red', linestyle=':', alpha=0.5, label='Coin flip (0.25)')
        ax.set_xticks(x)
    ...
    def _plot_ev_tracking(self, ax, runs):
        ...
        ax.plot(x, cum_predicted, 's-', color='#e67e22', markersize=6, label='Predicted EV')
        ax.plot(x, cum_actual, 'D-', color='#27ae60', markersize=6, label='Actual Points')
    ...
    if os.path.exists(state_path):
        try:
            ...
            if predictions and completed:
                ...
                print("[AccuracyTracker] Tournament state loaded but no predictions/completed games yet.")
        except (json.JSONDecodeError, KeyError) as e:
            print(f"[AccuracyTracker] Could not parse tournament state: {e}")
    else:
        print(f"[AccuracyTracker] No tournament state file at {state_path}. "
